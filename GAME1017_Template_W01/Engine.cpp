...
bool Engine::Init(const char* title, int xpos, int ypos, int width, int height, int flags)
{
	...
	SDL_Texture* m_pTileText = IMG_LoadTexture(m_pRenderer, "Img/Tiles.png");
	SDL_Texture* m_pPlayerText = IMG_LoadTexture(m_pRenderer, "Img/Maga.png");
	FontManager::RegisterFont("img/ltype.ttf", "tile", 10);
	Player* m_pPlayer = new Player({ 0,0,32,32 }, { (float)(16) * 32, (float)(12) * 32, 32, 32 }, m_pRenderer, m_pPlayerText, 0, 0, 0, 4);
	Sprite* m_pBling = new Sprite({ 224,64,32,32 }, { (float)(16) * 32, (float)(4) * 32, 32, 32 }, m_pRenderer, m_pTileText);
	ifstream inFile("Dat/Tile data.txt");
	int m_tiles;
	if (inFile.is_open())
